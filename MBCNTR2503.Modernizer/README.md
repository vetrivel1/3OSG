...
- **Partially covered (needs work):**
  - EBCDIC→ASCII and split: Improve `.dat.asc` and `.asc.11.1.s` fidelity (see per-job stats). Validate text vs numeric conversions near mixed zones and packed windows.
...
- **Immediate next actions (no hardcoding; use overrides):**
  - Externalize and validate S‑record conversion nuances in `EbcdicProcessor` using schema metadata; add targeted tests for the 53% S‑record gap.
  ...
  - Tighten key enrichment parity by verifying field offsets, zero‑fill, and counter formatting.
