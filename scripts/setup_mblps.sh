#!/usr/bin/env bash
set -euo pipefail
# Build, compile schema (mblps), and run EBCDIC->ASCII for either ALL jobs or a specified job.
# Usage:
#   bash scripts/setup_mblps.sh             # runs for ALL jobs
#   bash scripts/setup_mblps.sh 80362       # runs for a single job
...
for JOB in "${JOBS[@]}"; do
  ...
  echo "[$COUNTER/$total] Running EBCDIC->ASCII for job $JOB (schema=mblps) ..."
  dotnet run --project "$CLI_PROJECT" -- ebcdic-to-ascii \
    --job "$JOB" \
    --input "$IN_FILE" \
    --out "$JOB_OUT" \
    --schema "$SCHEMA_DIR" \
